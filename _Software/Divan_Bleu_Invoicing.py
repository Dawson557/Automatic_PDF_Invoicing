...
#Close Excel to make sure we don't run into issues later
...
sheet_handler = Spreadsheet_Controller.Sheet_Handler(month, year)
...
def main():
	...
	data_package = Data_Packager.Therapist_Data_Package(therapist_data, month_data, TPS_rate, TVQ_rate)
	...
	for therapist in data_package.therapists:
		...
		if (opt.rent):
			...
			for service in services:
				...
				rate, revenue, commission, quantity = data_package.get_drawing_values(therapist, service)
				if (rate > 0):
		...
		if (opt.email) and (total != 0):
			...
			subject = "Commission de {}".format(month)
			if opt.rent:
				subject = "Loyer de {}".format(month)
		...
		#update invidual spreadsheets
		excel_fn = therapist + os.sep + therapist + "_totals.xlsx"
		sheet_handler.individual_report(opt.rent, therapist, excel_fn, data_package)

	summary_fn = 'Summary Report.xlsx'
	if not os.path.isfile(summary_fn):
		Spreadsheet_Controller.create_workbook(summary_fn, year)

	sheet_handler.summary_report(summary_fn, opt.rent)
